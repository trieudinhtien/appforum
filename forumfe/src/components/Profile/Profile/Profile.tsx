...
import Navigation1 from "../../Navigation/Navigation";
  ...
  const _onChangeAvatar = () => {
    ...
    if (avatarRef?.current?.files) {
      formData.append(
        ...
        `avatar${Number(new Date())}.jpg`
      );
      saveImg(formData)
        .then((res: { path: string }) => {
          localStorage.setItem(
            "user",
            JSON.stringify({ ...user, avatar: res.path })
          );
  ...
  const _onChangeCover = () => {
    ...
    if (coverRef?.current?.files) {
      formData.append(
        ...
        `cover${Number(new Date())}.jpg`
      );
      saveImg(formData)
        .then((res: { path: string }) => {
          localStorage.setItem(
            "user",
            JSON.stringify({ ...user, cover: res.path })
          );
    ...
    <AuthGuard moveTo="/login">
      ...
      <div className={styles.profile}>
        <div className={styles.profile_inner}>
          <img
            className={styles.profile_inner_img}
            src={user && user.cover}
            alt="cover"
          />
          <div>
          ...
          <img
            className={styles.profile_inner_avatar}
          ...
          <div
            className={
              "d-flex align-items-center " + styles.profile_inner_description
            }
          >
            <div
              className={
                "d-flex col align-items-center " + styles.description_inner
              }
            >
            ...
            <div className={"text-center col " + styles.description_inner}>
              <div className={"" + styles.des_name}>
                ...
                <i
                  className="fas fa-check-circle pl-2"
                  style={{ fontSize: "20px", color: "#1DA1F2" }}
                ></i>
              </div>
              <div className={"" + styles.des_email}>{user.email}</div>
            </div>
            <div
              className={
                "text-center col d-flex justify-content-center " +
                styles.description_inner
              }
            >
              <a
                className={styles.des_social}
                href={
                  user.socialMedia?.facebook ? user.socialMedia.facebook : "#"
                }
                style={{ backgroundColor: "#3763d2" }}
              >
              ...
              <a
                className={styles.des_social}
                href={
                  user.socialMedia?.instagram ? user.socialMedia.instagram : "#"
                }
                style={{ backgroundColor: "#f8468d" }}
              >
              ...
              <a
                className={styles.des_social}
                href={
                  user.socialMedia?.youtube ? user.socialMedia.youtube : "#"
                }
